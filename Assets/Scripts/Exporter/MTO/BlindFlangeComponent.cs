...
namespace MTO
{
  public class BlindFlangeComponent : IPipingComponent
  {
    private readonly BlindFlange _flange;
    private readonly LocalCodSys3d _cod;

    public BlindFlangeComponent( BlindFlange flange, LocalCodSys3d cod )
    {
      _flange = flange;
      _cod = cod;
    }
    ...
    public Vector3d Axis
    {
      get
      {
        var axis = -_flange.WeldTermConnectPoint.Vector;
...
//        var unit = _flange.Document.LengthUnits.PositionUnit;
        ...
        return new Vector3d[] { _flange.GetWorldPosition() * scale,
                                ( _flange.WeldTermConnectPoint.GlobalPoint + Axis * _flange.Length ) * scale,
