...
{
  public class ConcentricPipingReducerCombinationBodyCreator : BodyCreator<ConcentricPipingReducerCombination, Body>
  {
    public ConcentricPipingReducerCombinationBodyCreator( Entity _entity ) : base( _entity ) { }

    protected override void SetupMaterials( Body body, ConcentricPipingReducerCombination reducer )
    {
    ...
    protected override void SetupGeometry( Body body, ConcentricPipingReducerCombination reducer )
    {
    ...
    private void SetupInsulation( GameObject go, ConcentricPipingReducerCombination reducer, Vector3d largeTerm, Vector3d smallTerm, double length )
    {
    ...
    private ( Vector3[] vertices, int[] triangles ) CreateTaperPolygon( float largeRadius, float smallRadius, float length )
    {
      ...
      for ( int i = 0; i < vertNum; ++i ) {
        ...
        vertices[i] = new Vector3( length * 0.5f, largeRadius * cosTheta, largeRadius * sinTheta );
        vertices[i + vertNum] = new Vector3( length * -0.5f, smallRadius * cosTheta, smallRadius * sinTheta );
