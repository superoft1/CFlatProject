...
{
  [Entity( EntityType.Type.EccentricPipingReducerCombination )]
  public class EccentricPipingReducerCombination : Component, ILinearComponent
  {
    ...
    public EccentricPipingReducerCombination( Document document ) : base( document )
    {
      ...
      ComponentName = "EccentricPipingReducer" ;
    }
    ...
    public override void CopyFrom( ICopyable another, CopyObjectStorage storage )
    {
      ...
      var entity = another as EccentricPipingReducerCombination;
    ...
    private void UpdateCombination()
    {
      try {
        ...
        var prevDiameter = LargeDiameter;
        var prevPoint = LargeTermConnectPoint.Point;
        foreach ( var record in records.Skip( 1 ) ) {
          var diameter = DiameterFactory.FromNpsMm( record.NPS_L_mm );
          var point = prevPoint - record.Height.Millimeters() * Axis - Reference * ( prevDiameter - diameter.OutsideMeter ) * 0.5;
          _innerConnectPoints.Add( ( point, diameter ) );
          prevDiameter = diameter.OutsideMeter;
          prevPoint = point;
        }
...
//          Debug.LogError( $"Valid eccentric reducer combination not found. {largeRecord.InchiStr} x {smallRecord.InchiStr} in" );
//        }
    ...
    {
      var refDis = ( LargeDiameter - SmallDiameter ) * 0.25;
      LargeTermConnectPoint.SetPointVector( 0.5 * length * Axis + Reference * refDis, Axis );
      SmallTermConnectPoint.SetPointVector( -0.5 * length * Axis - Reference * refDis, -Axis );
